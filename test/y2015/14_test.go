...
func TestDay14PartOne(t *testing.T) {
	...
	for i, v := range expected {
		if r := d.PartOne(tools.ReadFileBytes(fmt.Sprintf("../testdata/y2015/14/1.%v.txt", i))); r != v {
...
func TestDay14PartTwo(t *testing.T) {
	...
	for i, v := range expected {
		if r := d.PartTwo(tools.ReadFileBytes(fmt.Sprintf("../testdata/y2015/14/2.%v.txt", i))); r != v {
