...
func TestDay16PartOne(t *testing.T) {
	...
	for i, v := range expected {
		if r := d.PartOne(tools.ReadFileBytes(fmt.Sprintf("../testdata/y2015/16/1.%v.txt", i))); r != v {
...
func TestDay16PartTwo(t *testing.T) {
	...
	for i, v := range expected {
		if r := d.PartTwo(tools.ReadFileBytes(fmt.Sprintf("../testdata/y2015/16/2.%v.txt", i))); r != v {
