...
func TestDay07PartOne(t *testing.T) {
	...
	for i, v := range expected {
		if r := d.PartOne(tools.ReadFileBytes(fmt.Sprintf("../testdata/y2015/07/1.%v.txt", i))); r != v {
...
func TestDay07PartTwo(t *testing.T) {
	...
	for i, v := range expected {
		if r := d.PartTwo(tools.ReadFileBytes(fmt.Sprintf("../testdata/y2015/07/2.%v.txt", i))); r != v {
