...
func TestDay03PartOne(t *testing.T) {
	...
	for i, v := range expected {
		if r := d.PartOne(tools.ReadFileBytes(fmt.Sprintf("../testdata/y2015/03/1.%v.txt", i))); r != v {
...
func TestDay03PartTwo(t *testing.T) {
	...
	for i, v := range expected {
		if r := d.PartTwo(tools.ReadFileBytes(fmt.Sprintf("../testdata/y2015/03/2.%v.txt", i))); r != v {
