...
func TestDay02PartOne(t *testing.T) {
	...
	for i, v := range expected {
		if r := d.PartOne(input.ReadFileBytes(fmt.Sprintf("../testdata/y2015/02/1.%v.txt", i))); r != v {
...
func TestDay02PartTwo(t *testing.T) {
	...
	for i, v := range expected {
		if r := d.PartTwo(input.ReadFileBytes(fmt.Sprintf("../testdata/y2015/02/2.%v.txt", i))); r != v {
