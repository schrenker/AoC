...
func main() {
	...
	input.GetInput(os.Args[1], os.Args[2])

	if i, _ := strconv.Atoi(args[2]); i == 1 {
		fmt.Println(challenges[args[0]+"/"+args[1]].PartOne(input.ReadFileBytes(input.GetDefaultInputPath())))
	} else if i == 2 {
		fmt.Println(challenges[args[0]+"/"+args[1]].PartTwo(input.ReadFileBytes(input.GetDefaultInputPath())))
	} else {
