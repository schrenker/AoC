...
func TestDay17PartOne(t *testing.T) {
	...
	for i, v := range expected {
		if r := d.PartOne(tools.ReadFileBytes(fmt.Sprintf("../testdata/y2015/17/1.%v.txt", i))); r != v {
...
func TestDay17PartTwo(t *testing.T) {
	...
	for i, v := range expected {
		if r := d.PartTwo(tools.ReadFileBytes(fmt.Sprintf("../testdata/y2015/17/2.%v.txt", i))); r != v {
